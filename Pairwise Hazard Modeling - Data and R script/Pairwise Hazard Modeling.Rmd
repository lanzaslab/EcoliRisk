...
inoc_date: Date that certain calves were inoculated. If NA, individuals were not inoculated with *E. coli*, and any observed *E. coli* colonization was the result of successful transmission.
...
#Add week 0 and week 52 data to the data set (weeks when only the inoculated calves were infectious).
inoc_2017 <- unique(calf_transmission.updated[which(calf_transmission.updated$year == 2017 & is.na(calf_transmission.updated$inoc_date) == FALSE), match("calf_id", colnames(calf_transmission.updated))])
inoc_2018 <- unique(calf_transmission.updated[which(calf_transmission.updated$year == 2018 & is.na(calf_transmission.updated$inoc_date) == FALSE), match("calf_id", colnames(calf_transmission.updated))])
inoc_frame <- data.frame(
  
  year = c(rep(2017, 5), rep(2018, 5)), 
  calf_id = c(inoc_2017, inoc_2018),
  REDI_id = NA, #no need to import the REDI IDs
  inoc_date = c(rep("05-22-2017",5), rep("05-21-2018", 5)),
...
calf_transmission.updated <- data.frame(data.table::rbindlist(list(inoc_frame, calf_transmission.updated))) #bind the data together
```
